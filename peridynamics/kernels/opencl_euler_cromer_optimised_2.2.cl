...
// Calculate bond forces, reduce into nodal forces, and update acceleration
__kernel void
	UpdateAcceleration(
	...
	)
{
	...
	if ((global_id < (PD_NODE_NO * local_size)) && (local_id >= 0) && (local_id < local_size))
    {
        ...
        if (!local_id) 
		{
			...
			Uddn[DPN * node_no + 2] = (FCTypes[DPN * node_no + 2] == 2 ? ((local_cache_z[0] - PD_ETA * Udn[DPN * node_no + 2]) / PD_RHO) : ((local_cache_z[0] + FORCE_LOAD_SCALE * FCValues[DPN * node_no + 2] - PD_ETA * Udn[DPN * node_no + 2]) / PD_RHO));
        }
    }
}
